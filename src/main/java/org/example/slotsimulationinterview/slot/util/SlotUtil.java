...
public class SlotUtil {
    ...
    public static List<Payout> computePayout(Board board, int betAmount, SlotMachineConfig slotMachineConfig) {
        var payouts = new LinkedList<Payout>();
        var normalizedSpinSymbols = board.getNormalizedSpinSymbols();
        var paylines = slotMachineConfig.paylines();
        for (var payline : paylines) {
            int matchingCount = 1;
            for (int i = 0; i < payline.size() - 1; i++) {
                var current = normalizedSpinSymbols.get(payline.get(i));
                var next = normalizedSpinSymbols.get(payline.get(i + 1));
            ...
            if (matchingCount >= slotMachineConfig
                ...
                payouts.add(new Payout(
                        ...
                        matchingCount,
                        computeWin(betAmount, matchingCount, slotMachineConfig.payoutTable())));
            }
        }
    ...
    public static Map<String, Integer> getPayoutsAsMap(List<Payout> payouts) {
        return payouts.stream().collect(Collectors.toMap(Payout::payline, Payout::matchingCount));
    }
